...
- Define the adapter interfaces where they are consumed.
- These adapter interfaces should abstract driver or library differences for the same backend (for example, switching
  from `lib/pq` to `pgx`, or `go-redis` to `redigo`), not high‑level datastore changes such as PostgreSQL to MongoDB.
